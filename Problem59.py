...
# a file containing the encrypted ASCII codes, and the knowledge that the plain text must 
...
# It suffices to check all 26*26*26 = 17,576 possible 3-letter keys and store the ones that
# decrypt the cipher text(Problem59_cipher.txt) into a reasonable deciphered text.
# It takes a bit of guessing and checking to determine what characters are acceptable 
# in a reasonable deciphered text, but after going through the most common characters 
# in typical English text, we can find that only one key provides a reasonable answer.

import time
start_time = time.time()   #Time at the start of program execution
 
f = open("Problem59_cipher.txt", "r")
